	...
	public class TQGCategorySelector:MonoBehaviour
	{
		...
		public void StartGame()
		{
			...
			if ( categories[currentCategory].GetComponent<TQGDynamicXML>() ) 
			{
				...
				gameController.GetComponent<QuizController>().dynamicXML = newDynamicXML;
			}
			...
			if ( currentCategory >= categoriesToVictory )
			{
				...
				gameController.GetComponent<QuizController>().victoryCanvas = victoryCanvas;

				// Set the victory sound in the quiz as the sound to be played when we finish this category
				//gameController.GetComponent<QuizController>().soundVictory = soundVictory;
			}
		}
